...
export function ContributionGraph({
  ...
  // Determine the active year based on the latest data point if not explicitly provided
  const activeYear = useMemo(() => {
    if (year) return year;
    if (data && data.length > 0) {
      // Find the most recent date in the data array
      const latestDateStr = data.reduce((latest, current) => {
        return current.date > latest ? current.date : latest;
      }, data[0].date);
      if (latestDateStr) {
        return new Date(latestDateStr).getFullYear();
      }
    }
    ...
    for (let week = 0; week < 53; week++) {
      for (let day = 0; day < 7; day++) {
        ...
        const isPreviousYearDecember =
          currentDate.getFullYear() === activeYear - 1 &&
          currentDate.getMonth() === 11
        const isNextYearJanuary =
          currentDate.getFullYear() === activeYear + 1 && currentDate.getMonth() === 0
  ...
  // Auto-scroll to the far right on mount so the user sees the latest days immediately
    ...
    <div className={`contribution-graph ${className}`}>
      <div className="overflow-x-auto no-scrollbar" ref={scrollContainerRef}>
        <table className="border-separate border-spacing-1 text-xs">
          <caption className="sr-only">Contribution Graph for {activeYear}</caption>
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        } hover:ring-foreground/20 hover:ring-2 transition-all`}
                      />
                    </td>
                  )
                })}
